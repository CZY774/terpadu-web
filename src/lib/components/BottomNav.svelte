...
<nav class="fixed bottom-0 left-0 right-0 bg-[#FF6B35] rounded-t-[2.5rem] flex justify-around py-4 px-4">
	...
	<a href="/schedule" class="flex flex-col items-center text-white {$page.url.pathname.startsWith('/schedule') ? 'opacity-100' : 'opacity-70'}">
		<svg class="w-9 h-9" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<button onclick={onAddClick} class="flex flex-col items-center text-white -mt-3">
		<div class="w-16 h-16 bg-[#FF6B35] rounded-full flex items-center justify-center border-4 border-[#F5F1E8]">
			<svg class="w-9 h-9" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4" />
			</svg>
		</div>
	</button>
	<a href="/tasks" class="flex flex-col items-center text-white {$page.url.pathname.startsWith('/tasks') ? 'opacity-100' : 'opacity-70'}">
		<svg class="w-9 h-9" fill="none" stroke="currentColor" viewBox="0 0 24 24">
